    ...
    public function up(): void
    {
        DB::statement("
            ...
            select
                ...
                left join 
                (select patient_id , json_agg(cp2.med_num) as medical_no from company_patient cp2 inner join companies c2 on cp2.company_id = c2.id GROUP BY patient_id) cow2 on p.id = cow2.patient_id
                left join 
                ...
                left join 
                (select addressable_id , json_agg(aty.name) as type_address from addresses ad inner join address_types aty on ad.address_type_id = aty.id WHERE ad.addressable_type::text = 'App\Models\Patient'::text GROUP BY addressable_id) coww2 on p.id = coww2.addressable_id
                left join 
                (select addressable_id , json_agg(ad.address) as address from addresses ad WHERE ad.addressable_type::text = 'App\Models\Patient'::text GROUP BY addressable_id) ad1 on p.id = ad1.addressable_id
                left join 
                (select addressable_id , json_agg(ad.apt_suite) as apt_suite from addresses ad WHERE ad.addressable_type::text = 'App\Models\Patient'::text GROUP BY addressable_id) ad2 on p.id = ad2.addressable_id
                left join 
                (select addressable_id , json_agg(ad.zip) as zip from addresses ad WHERE ad.addressable_type::text = 'App\Models\Patient'::text GROUP BY addressable_id) ad3 on p.id = ad3.addressable_id
                left join 
                (select addressable_id , json_agg(ad.city) as city from addresses ad WHERE ad.addressable_type::text = 'App\Models\Patient'::text GROUP BY addressable_id) ad4 on p.id = ad4.addressable_id
                left join 
                (select addressable_id , json_agg(ad.state) as state from addresses ad WHERE ad.addressable_type::text = 'App\Models\Patient'::text GROUP BY addressable_id) ad5 on p.id = ad5.addressable_id
                left join 
                (select addressable_id , json_agg(ad.country) as country from addresses ad WHERE ad.addressable_type::text = 'App\Models\Patient'::text GROUP BY addressable_id) ad6 on p.id = ad6.addressable_id
